...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun x => evenb x && (negb (leb x 7))) l.
...
Definition partition {X : Type}
                     ...
                     (l : list X)
                   : list X * list X :=
  (filter (fun x => if test x then true else false) l, filter (fun x => if negb (test x) then true else false) l).
...
Lemma map_over : forall (X Y : Type) (f : X -> Y) (l1 l2: list X),
...
Proof.
  ...
  induction l.
  - simpl. reflexivity. 
  - simpl. rewrite -> map_over. rewrite <- IHl. reflexivity. 
Qed.
(** [] *)
...
Compute (fold_map (fun x => x + 1)) [0; 1; 2].
